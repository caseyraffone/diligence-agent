...
```bash
git clone <your-remote> credential-integrity-agent
cd credential-integrity-agent
npm install
cp .env.example .env
...
sudo -u postgres psql -c "CREATE ROLE cia LOGIN PASSWORD 'cia_local_dev' CREATEDB;"
sudo -u postgres psql -c "CREATE DATABASE cia_dev OWNER cia;"
sudo -u postgres psql -c "CREATE DATABASE cia_test OWNER cia;"
```
...
createuser cia --createdb && createdb cia_dev -O cia && createdb cia_test -O cia
psql -d cia_dev -c "ALTER ROLE cia PASSWORD 'cia_local_dev';"
```
